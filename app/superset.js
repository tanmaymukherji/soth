...
soth.superset = {
  ...
  render: async function (containerId) {
    ...
    // Partner coverage section — only for authenticated users
    ...
    if (user && profile) {
      ...
      const orgsToShow = profile.role === 'soth_admin'
        ? orgs  // Admin sees all
        : orgs.filter(o => o.id === profile.org_id);  // Partner sees only their org
      ...
      html += '<div class="partner-coverage"><p class="empty-state"><a href="login.html">Log in</a> to view partner coverage data.</p></div>';
    }
